...
const componentMatches = jsMain.matchAll(
  /Vivere.register\(['"]([A-z]+)['"], ([A-z]+)\);/g,
);
...
const importMatches = jsMain.matchAll(
  /import ([A-z]+) from ['"]([A-z0-9-./@]+)["']/g,
);
for (let match of importMatches) {
  ...
  for (let componentName in components) {
    ...
    if (impName === importName) {
      ...
      const exportedContent = jsFileContent.match(/export [A-z ]* {(.|\s)*/)[0];

      const attributes = [];
      const attributesMatcher = exportedContent.matchAll(/^\s{2}([A-z]+):/gm);
...
export default {
  parseDirectives(fileName) {
    ...
    const includeMatches = fileContent.matchAll(/<%- include\('([\/A-z-]+)/g);
    for (let match of includeMatches) {
      const filePart = match[1];
      const parsed = this.parseDirectives(`examples/includes${filePart}.ejs`);

      for (let partial of parsed.partials) partials.push(partial);

      for (let script of parsed.scripts) scripts.push(script);

      for (let directive of parsed.directives) directives.push(directive);

      for (let property of parsed.properties) properties.push(property);

      partials.push(`${filePart}.ejs`);
    }

    // Parse HTML directives
    const directiveMatches = fileContent.matchAll(/\s(v-[A-z]+)/g);
    ...
    if (fileContent.match(/\s\#[A-z-]+/)) directives.push('v-data');

    if (fileContent.match(/\s\$[A-z-]+/)) directives.push('v-func');

    if (fileContent.match(/\s\*sync/)) directives.push('v-sync');

    if (fileContent.match(/\s\*ref/)) directives.push('v-ref');

    if (fileContent.match(/\s:[A-z-]+/)) directives.push('v-attr');

    if (fileContent.match(/\s@[A-z-]+/)) directives.push('v-on');

    if (fileContent.match(/\s\*for/)) directives.push('v-for');

    if (fileContent.match(/\s\*if/)) directives.push('v-if');

    if (fileContent.match(/\s\*else-if/)) directives.push('v-else-if');

    if (fileContent.match(/\s\*else/)) directives.push('v-else');

    const syncMatches = fileContent.matchAll(/\s\*sync+/g);
    if (syncMatches?.length) directives.push('v-sync');
    ...
    [
      ...
      for (let match of componentMatches) {
        ...
        if (componentDetails != null) {
          ...
          if (scriptContent.match(/\$attach\(/)) properties.push('$attach');

          if (scriptContent.match(/\$refs/)) properties.push('$refs');

          if (scriptContent.match(/\$dispatch\(/)) properties.push('$dispatch');
        }
      }
    });
    ...
    return {
      ...
      scripts: scripts.filter(uniqueness).sort(),
      partials: partials.filter(list.uniqueness).sort(),
      properties: properties.filter(list.uniqueness).sort(),
    };
  },
};
